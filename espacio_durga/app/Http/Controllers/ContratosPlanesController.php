...
class ContratosPlanesController extends Controller
{
    ...
    {
        $contratosVigentes = ContratoPlan::withTrashed()
            ->with([
            ...
            ->where('estado', 1)
            ->orderBy('inicio_mensualidad', 'desc')
            ->get();

        $contratosFinalizados = ContratoPlan::withTrashed()
            ->with([
                'alumno' => function ($query) {
                    $query->withTrashed()
                        ->with([
            ...
            ->where('estado', 0)
            ->orderBy('fecha_termino_contrato', 'desc')
            ->get();
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $referenced = $contrato->asistencias()->exists();
        if ($request->registros) {
            if ($referenced) {
                return redirect()->route('contratos.index', $contrato->rut_alumno)->withErrors('No puede descontar los ingresos de un contrato que tiene asistencias. Borre las asistencias primero.');
            } else {
                $this->finalizarContrato($contrato);
                $contrato->delete();
            }
